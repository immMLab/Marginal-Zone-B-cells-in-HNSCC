...
run_harmony <- function(input_sce){
  ...
  seuratObj <- RunHarmony(input_sce, "orig.ident") #harmony is runed per sample, i.e. group.by.vars (orig.ident)
...
CD4_markers_list =list(
  ...
  CXCL13_T=c('CXCL13','CCR5','PD-1','IL7R','FOXP3')
)  
...
Bcels_markers_list = list(
  ...
  Naive_B = c('MS4A1','IGHD','CD38'),
  B_mem = c('CD19', 'CD27'),
  GC_B = c('IL4R','TCL1A','LRMP','SUGCT','SEMA4A'),
  MZ_B = c("CR2","FCER2","CD1C","CD1D","IGHM"),
  IGA_plasm_B= c ( 'MZB1','IGHA1'),
  IGG_plasm_B= c ( 'MZB1','IGHG1'),
  To_check= c('CXCL13','CXCR5','FOXP3', 'ENTPD1','NT5E')
)  
...
if(sp=='human'){
   lapply(markers, function(x){
     ...
     ggsave(paste('check_for_',x,'.pdf'),height = h,limitsize = FALSE)
   })
  lapply(markers_list, function(x){
    ...
    ggsave(paste('check_for_',x,'.pdf'),width  = w,limitsize = FALSE)
  })
...
ggsave(paste('last_markers_and_umap.pdf'),width  = w,height = h,limitsize = FALSE)
...
ggsave(filename=paste0(pro,"Vlnplot1.pdf"),plot=p1,width = w,height = 5,limitsize = FALSE)
...
ggsave(filename=paste0(pro,"Vlnplot2.pdf"),plot=p2,width = w,height = 5,limitsize = FALSE)

p3=FeatureScatter(sce.all.int , "nCount_RNA", "nFeature_RNA", 
                  pt.size = 0.5)
ggsave(filename=paste0(pro,"Scatterplot.pdf"),plot=p3,limitsize = FALSE)
